...
def render():
    ...
    encoders_path = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                'encoders.pkl'))
    ...
    categorical_features = [
        "Transaction_ID","User_ID","Transaction_Type","Device_Type","Location",
    ...
    for feature in numeric_features:
        user_input[feature] = st.number_input(feature, value=0.0, key=f"num_{feature}")
    for feature in categorical_features:
        val = st.text_input(feature, key=f"text_{feature}")
        if val.strip() == "":
    ...
    if st.button("Predict"):
        try:
            input_features = [user_input[f] for f in numeric_features + categorical_features]
